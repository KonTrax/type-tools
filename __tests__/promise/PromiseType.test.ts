...
;{ // CASE: Works correctly
	...
	{
		type RES = Test<A>
		return expect.equals() as Equals<RES, B> & { IN :A, EXP :B, RES :RES }
	}
	...
	;TRU = expEqual({} as {
			EXP :1 |         Promise<1>  |             PromiseLike<1>
			IN  :1 | Promise<Promise<1>> | PromiseLike<PromiseLike<1>>
	})
	;TRU = expEqual({} as {
			EXP :1 |         Promise<2>  |             PromiseLike<3>
			IN  :1 | Promise<Promise<2>> | PromiseLike<PromiseLike<3>>
	})
	...
	;TRU = expEqual({} as {
			EXP : 1 | 2 | 5
					| Promise<3> | PromiseLike<4>
					| Promise<6> | PromiseLike<7>
			IN  : 1
